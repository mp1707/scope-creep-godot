...
const OPERATION_MULTIPLY_REMAINING_DURATION: int = 1
...
func calculate(
	...
	if not _target_stack_has_card_type(target_stack, state, content, interaction.required_target_card_type):
		return result
	if not interaction.required_active_recipe_id.is_empty() and target_stack.processing_state.active_recipe_id != interaction.required_active_recipe_id:
		return result
	...
	match interaction.operation:
		...
		OPERATION_MULTIPLY_REMAINING_DURATION:
			_calculate_remaining_duration_multiplier(result, moving_card_ids, target_stack, interaction)
		_:
			return ActiveProcessingInteractionResultScript.new() as ActiveProcessingInteractionResult
...
func _calculate_remaining_duration_multiplier(
	result: ActiveProcessingInteractionResult,
	moving_card_ids: PackedStringArray,
	target_stack: StackState,
	interaction: ProcessingInteractionDefinition
	...
	var max_applications: int = interaction.max_applications_per_drop
	var multiplier_per_card: float = interaction.remaining_duration_multiplier_per_card
	var applications: int = mini(moving_card_ids.size(), max_applications)
	if applications <= 0 or multiplier_per_card <= 0.0:
		return
	...
	result.applied = true
	result.progress_added = 0.0
	result.new_elapsed = processing.elapsed
	result.should_complete = false
...
func _interactions_match(left: ProcessingInteractionDefinition, right: ProcessingInteractionDefinition) -> bool:
	return left.operation == right.operation \
		...
		and is_equal_approx(left.remaining_duration_multiplier_per_card, right.remaining_duration_multiplier_per_card) \
		...
		and left.required_active_recipe_id == right.required_active_recipe_id \
